...
void CanBus::loop() {
    ...
    while (twai_receive( &rx_frame, 8 * portTICK_PERIOD_MS) == ESP_OK) {
...
void CanBus::processFrame(twai_message_t rx_frame, int frameCount) {
    ...
    if (RECV_STATUS_1 == ID) {
     ...
     if (RECV_PROCESS_SHORT_BUFFER_PROXY == ID) {
        ...
        Serial.printf("processing short buffer proxy, data[1]=%d, data[2]=%d, data[3]=%d", rx_frame.data[1], rx_frame.data[2], rx_frame.data[3]);
        for (int i = 2; i < rx_frame.data_length_code; i++) { // from 2!!! data[1]=1, data[2]=36, data[3]=101
            proxybuffer.push_back(rx_frame.data[i]);
        }
        unsigned short crc = crc16(proxybuffer.data(), proxybuffer.size());
